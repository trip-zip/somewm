...
luaA_dbus_connect_signal(lua_State *L)
{
    ...
    sig = signal_array_getbyname(&dbus_signals, name);
    if(sig) {
...
luaA_dbus_emit_signal(lua_State *L)
{
    ...
    bus_name = luaL_checkstring(L, 1);
    path = luaL_checkstring(L, 2);
    itface = luaL_checkstring(L, 3);
    name = luaL_checkstring(L, 4);
    dbus_connection = a_dbus_bus_getbyname(L, bus_name);
    msg = dbus_message_new_signal(path, itface, name);
    if (msg == NULL) {
