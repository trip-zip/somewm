...
| Tiled-client layout (tile / fair / max / corner / spiral / spiral.dwindle / floating) | `awful.layout.clay.*` presets, all `somewm.layout.descriptor` layouts whose `body = function(p)` returns a tree (slicing `p.clients`, reading `p.master_count` / `p.master_width_factor`), merged into the screen solve by `compose_screen`. `floating` emits a stack of self-positioned clients (no-op solve for positioning + bw2 round-trip preservation). `max.fullscreen` carries `bounds_source = "geometry"`, so `compose_screen` grafts its subtree as a root-attached container spanning `screen.geometry` (covering the wibar regions) instead of the workarea. `magnifier` stays a bespoke `arrange(p)` (it positions by focus, not by the tiled set): it writes each client's box to `p.geometries`, which `compose_screen` reflects as root-attached leaves like any descriptor-less layout; documented in `lua/awful/layout/clay/magnifier.lua`. | Lua |
| Per-client frame (4 borders + 4 titlebars + surface + shadow) | `clay_apply_client_frame()` in `clay_layout.c` | C |
| Widget hierarchy (wibox.layout.*) | All wibox layouts route through Clay. `flex` / `fixed` use flex distribution including `spacing_widget` (interleaved spacer leaves). `align` covers all three expand modes via Clay: `inside` / `outside` flex-grow; `none` is `layout.stack` with absolute child positions. `ratio` default uses flex distribution; non-default strategies (justify/center/spacing/left/right) and `spacing_widget` compute slot positions in Lua (void redistribution) then emit via `layout.stack` with absolute child positions. `stack` (z-order with offsets), `manual` (user-supplied points), and `grid` (cell math) all emit via `layout.stack`. `overflow` computes scroll positions in Lua and emits its visible children as a `layout.stack` of absolutely-placed nodes (clipping stays in `before_draw_children`); `wibox.widget.systray` solves a padded grid of fixed icon cells (a row-of-columns when horizontal, a column-of-rows when vertical), the same descriptor feeding `:layout` and `:fit`. | Lua via `somewm.layout.solve` |
...
| Notification stacking (`naughty.layout.box`) | First notification routes through `placement[position]` (Clay-backed alias); subsequent notifications chain `placement.next_to` (a non-widget anchor), intentionally bespoke per the non-widget try-and-fit row below. | Lua via `somewm.placement.solve` (first) + bespoke search (rest) |
...
| Widget-anchored placement | `awful.popup`, `awful.tooltip` (`popup:move_next_to(widget)`, outside-mode tooltips) | The per-anchor offset math now goes through Clay: the surface is a floating drawin attached to the anchor widget's element by id (`attach_to_element_id` -> `CLAY_ATTACH_TO_ELEMENT_WITH_ID`), with `attach_points` chosen from the preferred position / anchor. Only the fit/flip **selection** stays in Lua (`placement.next_to_attach`): Clay's float attach is pure parent-corner-minus-element-corner arithmetic with no viewport fit (`third_party/clay.h`), so deciding which preferred side fits and flipping when it does not is app policy with no Clay API. |
| Try-and-fit (non-widget anchors) | `placement.next_to` | A client / geometry / mouse anchor has no Clay element to attach to, so it keeps the full search-with-short-circuit. The 9-way offset math is shared with the widget-anchor row via the extracted `next_to_select`; only this path still emits the absolute geometry itself. |
...
| Scrollable / viewport-relative | `awful.layout.suit.carousel` | Unbounded strip with a viewport offset; positions depend on scroll state, not on a finite parent rect, so carousel computes them itself and animates them frame-by-frame. At rest it writes each on-screen client's box to `p.geometries`, which `compose_screen` reflects as a root-attached leaf, so the visible columns are in the tree and reach the assert (allow-listed). Scrolled-out columns stay applied by carousel itself, off-tree: Clay culls a fully off-screen element (no render command), so reflecting one would leave a zeroed result that the apply pass would write as a 1x1 box, so `reflect_geometries` skips off-screen columns. The reflection is also skipped while a scroll animates (the snapshot would lag the frame-by-frame motion). The position math stays bespoke; only the at-rest, on-screen apply is shared. |
...
Every layout now applies its clients through `clay_apply_all()` from the one screen solve. Descriptor layouts (tile / fair / spiral / corner / max / floating) lay their clients in the solve directly; descriptor-less layouts (magnifier, carousel, a user's `function arrange(p)`) have their arrange output reflected into the same solve as root-attached leaves. So every client reaches the assert.

Widget-anchored popups and tooltips are part of the same solve: each is a Clay floating drawin attached to its anchor widget's element (`provider_popups`), so `clay_apply_all()` applies its box and it satisfies the assert natively, with no allow-list entry (like a wibar, unlike carousel). The anchor widget carries a stable, content-independent Clay id (`base.widget_anchor_id`) so the surface's `attach_to_element_id` resolves to the same element across solves. `awful.menu` stays a bespoke leaf: its wibox geometry is assigned directly in `set_coords` (off `s.workarea`), never produced by a Clay result, so the C assert (which only checks solve results) never sees it and it needs no allow-list entry.

The allow-list is a single category, not a per-layout-name list: a divergence is expected on a **descriptor-less** screen and a violation on a **descriptor** screen. `_somewm_clay.assert_allowed` (in `lua/awful/layout/init.lua`) returns true exactly when the screen's layout has no descriptor; the C assert consults it only after a mismatch, so the common matching case stays per-frame-free.

The reasoning: a descriptor layout lays its clients in the solve and must match the scene exactly, so a divergence means the conversion is not yet exact and aborts. A descriptor-less layout's boxes come from bespoke or user code that may impose its own constraints (size hints, edge clipping), so a divergence there is a documented leaf. A well-behaved reflection matches by construction (C applies the solved box; the assert inverts the same border math), so most descriptor-less clients pass anyway. The category covers the genuine divergences:
...
- carousel: an on-screen column reflects at its true box (Clay does not clip partially-visible elements), so the apply is a no-op against what carousel already applied; a size-hint-adjusted client can still diverge (allow-listed). Fully scrolled-out columns and in-flight scroll animation are reflected nowhere (off-tree), so they are not asserted at all.

On a **descriptor** screen the per-client constraint cases (aspect-ratio, thin clamp) are the only remaining un-allowed divergence; closing that gap (a per-client predicate keyed on the divergence cause) is a follow-up, and the descriptor abort suites steer clear of such clients. The documented leaves the allow-list covers (carousel, magnifier, any descriptor-less `arrange(p)`) match the descriptor-less category exactly: nothing is allow-listed that is not also a documented leaf, and no documented leaf is missing from the category.
...
### Custom layouts: `arrange(p)` is a documented hatch

Existing user layouts (`function arrange(p) ... end` populating `p.geometries[c] = {...}`) keep working unchanged, and this is a supported public shape, not a deprecation. The per-screen recompute runs the layout's `arrange(p)`, then `compose_screen` reflects the boxes it wrote to `p.geometries` into the one screen solve as root-attached leaves, so the clients are applied in C from the tree like every other layout. The box written to `p.geometries[c]` is the same outer box such layouts always wrote; the reflection subtracts the tag's gap and the client border exactly as the old imperative apply loop did, so geometry is unchanged. (The other shape is a `somewm.layout.descriptor` that returns a tree, which merges instead of reflecting.)
