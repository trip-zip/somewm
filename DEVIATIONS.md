...
**Attachment hosts share equal areas.** A popup, tooltip, launcher or notification with no decoration inset owns its drawable contribution directly. Rectangular attachment borders without shadows use Clay border paint at the solved size instead of a previous-size raster. Original widget objects, content areas, border input passthrough and drawin opacity behavior are preserved. Shaped borders, shadows and distinct inset content areas retain their existing paths pending the rest of task 10.
...
Carousel (including vertical) still use `awful.layout` geometry
while their required 2.1 declaration work remains in progress. Each client in
these layouts is currently a float at its computed box, marked `derived` in
the dump. Tile, spiral, dwindle, max, fullscreen, magnifier, fair and corner layouts have no such computed client boxes. Floating geometry is a user input, fullscreen grows to
