...
| `awful/client.lua` | `c.type or "normal"` fallback; `awful.client.resolve_icon()` sets `c.icon` at manage time from the icon theme or a matching `.desktop` file | Native Wayland clients may not set window type and have no icon protocol (no `_NET_WM_ICON` equivalent) |
...
Wiboxes and drawins have a matching `shadow` property.
