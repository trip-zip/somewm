...
luaA_loadrc(void)
{
	...
	for (i = 0; config_paths[i] != NULL; i++) {
		...
		{
			prescan_result_t prescan_result = {0};
			if (!check_config_prescan(config_paths[i], NULL, &prescan_result)) {
				if (prescan_result.found_fatal) {
					globalconf.x11_fallback.config_path = prescan_result.config_path;
					globalconf.x11_fallback.line_number = prescan_result.line_number;
					globalconf.x11_fallback.pattern_desc = prescan_result.pattern_desc;
					globalconf.x11_fallback.suggestion = prescan_result.suggestion;
					globalconf.x11_fallback.line_content = prescan_result.line_content;
				}
