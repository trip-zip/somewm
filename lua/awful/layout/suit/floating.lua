    ...
    for _, v in ipairs(state.buttons) do
      if v then
        ...
        elseif corner == "bottom_left" then
          ng = { x = state.x, width = (g.x + g.width) - state.x, height = state.y - g.y }
        elseif corner == "top_left" then
          ng = { x = state.x, width = (g.x + g.width) - state.x, y = state.y, height = (g.y + g.height) - state.y }
        else
          ng = { width = state.x - g.x, y = state.y, height = (g.y + g.height) - state.y }
        end
        ...
        elseif corner == "top_left" then
          ng = { x = (g.x + g.width) - rg.width, y = (g.y + g.height) - rg.height }
        else
