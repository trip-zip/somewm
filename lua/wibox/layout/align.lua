...
-- layout_clay's sizing. expand=="none" uses absolute stack positions, which the
-- merged path doesn't graft; return nil so the align degrades to a single leaf.
function align:layout_node(context, width, height)
    if self._private.expand == "none" then
        return nil
    end
    ...
    local function main_fit(w)
        local fw, fh = base.fit_widget(self, context, w, width, height)
        return is_y and fh or fw
    end

    local children = {}
    local function add(w, props)
        local node = base.widget_to_node(self, context, w, width, height, props)
        if node then children[#children + 1] = node end
    end

    if mode == "outside" then
        if first  then add(first,  { grow = true }) end
        if second then add(second, fixed_props(main_fit(second))) end
        if third  then add(third,  { grow = true }) end
    else
        if first  then add(first,  fixed_props(main_fit(first))) end
        if second then
            add(second, { grow = true })
        elseif first and third then
            children[#children + 1] = layout.row { grow = true }
        end
        if third  then add(third,  fixed_props(main_fit(third))) end
    end

    return outer { widget = self, children }
end
