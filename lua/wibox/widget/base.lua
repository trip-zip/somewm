...
-- cannot import this module, so the recursion lives here next to `place_rects`.
-- @tparam widget parent The parent widget (for dependency tracking).
-- @tparam table context The widget context.
-- @tparam widget widget The widget to build a node for.
-- @tparam number width The available width.
-- @tparam number height The available height.
-- @tparam[opt] table props Sizing props merged onto the returned node.
-- @treturn[opt] table A `somewm.layout` node, or nil when invisible.
-- @staticfct wibox.widget.base.widget_to_node
function base.widget_to_node(parent, context, widget, width, height, props)
    record_dependency(parent, widget)
    ...
    local layout = require("somewm.layout")
    local node = widget.layout_node and widget:layout_node(context, width, height)
    if not node then
        node = layout.widget(widget, {})
    end
    ...
    if not node.props.id then
        local wn = widget.widget_name
        local leaf = type(wn) == "string" and wn:match("[%w_]+$")
        if leaf then
            node.props.id = "widget." .. leaf
        end
    end
