...
read_node(lua_State *L, int idx, struct widget_node *n)
{
	...
	lua_pop(L, 1);
	float parent = 0, own = 0;
	if (!read_number(L, idx, "parent", 0, 8, &parent)
			|| !read_number(L, idx, "own", 0, 8, &own)
			|| parent != floorf(parent) || own != floorf(own))
		return false;
	n->parent = parent;
	n->own = own;
	lua_getfield(L, idx, "passthrough");
	ok = ok && (lua_isnil(L, -1) || lua_isboolean(L, -1));
	n->passthrough = lua_isnil(L, -1) || lua_toboolean(L, -1);
	lua_pop(L, 1);
	...
	lua_pop(L, 1);
	if (n->image) {
		if (!read_number(L, idx, "aspect", 0, 1e6, &n->aspect)
				|| !read_number(L, idx, "image_width", 0, 1e6, &n->image_size[0])
				|| !read_number(L, idx, "image_height", 0, 1e6, &n->image_size[1]))
			return false;
		if (n->aspect > 0 && (n->image_size[0] <= 0 || n->image_size[1] <= 0)) {
			n->image_size[0] = cairo_image_surface_get_width((cairo_surface_t *)n->image);
			n->image_size[1] = cairo_image_surface_get_height((cairo_surface_t *)n->image);
		}
	...
	lua_getfield(L, idx, "track");
	n->track = lua_toboolean(L, -1);
	lua_pop(L, 1);
	lua_getfield(L, idx, "thumb");
	n->thumb = lua_toboolean(L, -1);
	lua_pop(L, 1);
...
read_tree(lua_State *L, int idx, struct widget_node *nodes, size_t *len,
	size_t *leaves, unsigned clip_by, unsigned *clips, int shapes, int anchors)
{
	...
	if (!lua_isnil(L, -1)) {
		...
		n->attach = (uint16_t)target;
	}
	lua_pop(L, 1);
	lua_pushvalue(L, idx);
	lua_pushinteger(L, *len);
	lua_rawset(L, anchors);
	if (n->shape) {
		if (n->image)
			return false;
	...
	for (size_t i = 0; ok && i < count; i++) {
		...
		ok = read_tree(L, lua_gettop(L), nodes, len, leaves, clip_by, clips, shapes, anchors);
		lua_pop(L, 1);
	}
	lua_pop(L, 1);
	return ok;
...
widget_nodes_set(lua_State *L, struct widget_tree *d, Monitor *m, int idx)
{
	...
	lua_newtable(L);
	bool ok = read_tree(L, idx, nodes, &len, &leaves, 0, &clips, shapes, lua_gettop(L));
	lua_pop(L, 1);
	size_t scrolls = 0;
	for (size_t i = 0; i < len; i++)
	...
	for (size_t i = 0; i < len; i++) {
		...
		if (same) {
			...
			/* A measurement declaration can remove every shape and recreate
