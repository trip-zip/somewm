...
local steps = {
    ...
    -- Clay's boxes: the drawable and ten elements. Stack/align share flow;
    -- place and clock bind to the same native float.
    function()
        local boxes = awesome._test_widget_boxes(bar.drawin)

        local align = bar.widget:get_children()[1]
        local children = align:get_children()
        ...
        assert_box(box(children[1]), { x = 0, y = 0, width = 142, height = BH }, "the left fixed")
        assert_box(box(children[2]), { x = 142, y = 0, width = 184, height = BH }, "the middle")
        assert_box(box(children[3]), { x = 326, y = 0, width = 74, height = BH }, "the right fixed")
        assert_box(box(bar.widget:get_children()[2]:get_children()[1]), { x = 165, y = 4, width = 70, height = 16 }, "the clock")
    ...
    function(count)
        ...
        assert(bar._drawable._clay_wired[bar.widget][1].element, "the stack did not stay converted")
        io.stderr:write("[PASS] fractional spacing rounds in the stack\n")
        return true
    end,
    function(count)
        if count == 1 then bar.widget.spacing = 0; return nil end
        assert(bar._drawable._clay_wired[bar.widget][1].element, "the stack did not return")
    ...
    function(count)
        ...
        assert(bar._drawable._clay_wired[bar.widget][1].element, "bgimage lost the original stack")
