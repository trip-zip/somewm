...
runner.run_async(function()
    ...
    for _,edge in ipairs{'top','bottom','left','right'} do
        ...
        local function check(name,combined)
            ...
            assert((dump:find(' drawable ',1,true)==nil)==combined,
                'inset content and equal-area host roots were confused')
            local boxes=awesome._test_widget_boxes(bar.drawin)
            local content=bar._drawable._clay_tree.children[1]
            assert(content.w=='grow' and content.h=='grow' and not content.float,
                'host allocation still uses a floating widget root')
            assert(not content.wmin and not content.hmin,
                'host allocation still carries previous-size minima')
