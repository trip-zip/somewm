...
local steps = {
    function(count_)
        ...
        -- One item per tag under the taglist, one per client under the
        -- tasklist's base layout, each down to its textbox, and the task's
    ...
    function(count_)
        if count_ == 1 then
            local _, list = nodes()
            local tags = {}
            for _, node in ipairs(items(list, "awful.widget.taglist")) do
                tags[#tags + 1] = tonumber(node.line:match("box (%d+),"))
            end
            assert(#tags == 3 and tags[2] > tags[1], "the items are not in a row")
            bar.drawin.drawable:emit_signal("button::press", tags[2] + 2, 12, 1, {})
            bar.drawin.drawable:emit_signal("button::release", tags[2] + 2, 12, 1, {})
            return nil
        end
