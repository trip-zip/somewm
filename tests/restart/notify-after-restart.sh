...
# for issue 444.
#
# The reload closes the shared GDBus session connection (luaa.c:5222-5230).
# GLib caches that connection in a GWeakRef cleared only on finalize, so every
# later Gio.bus_get_sync hands back the closed object, and naughty's
# bus_own_name rides that cache. Its owner id is discarded at creation
# (lua/naughty/dbus.lua:536), so the name can never be re-acquired either.
#
...
# xfail: the reload closes the shared session bus, so naughty never re-owns org.freedesktop.Notifications
...
check_bus_owner hr-notify "$NOTIFY_NAME" "the instance still owns $NOTIFY_NAME after the reload"
