...
local function block(bar)
    local d = bar.drawin
    local want = string.format("WIBAR screen %d %dx%d+%d+%d ",
    ...
    for _, line in ipairs(lines()) do
        if head then
            ...
            nodes[#nodes + 1] = line
        elseif line:find(want, 1, true) and line:match("^%s*WIBAR ") then
            head = line
        end
    end
    return head, nodes
...
local steps = {
    ...
    function(count)
        ...
        local head = block(bars.narrow)
        assert(head:find("w=grow h=fixed(20)", 1, true),
            "the reservation lost the full output width: " .. head)
        local area = bars.narrow._drawable._clay_wired[bars.narrow.widget][1].element.box
        assert(area.width == 300 and area.height == 20,
            "the original textbox lost its allocated input area")
    ...
    function(count)
        ...
        for mode, sizes in pairs(want) do
            ...
            local function original(node)
                for _, binding in clay.bindings(node) do
                    if binding.widget == bars[mode].widget then return node end
                end
                for _, child in ipairs(node.children or {}) do
                    local found = original(child)
                    if found then return found end
                end
            end
            local node = assert(original(bars[mode]._drawable._clay_tree))
            assert(#node.children == 3, 'align lost a declared allocation')
            for i = 1, 3 do
                local child = node.children[i]
                local got = 'w='..(child.w or 'fit')..' h='..(child.h or 'fit')
                assert(got == sizes[i], mode..' child '..i..': '..got)
            end
        end
