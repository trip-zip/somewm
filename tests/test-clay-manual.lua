...
local steps = {
    function(count)
        ...
        for line in awesome._clay_tree(s):gmatch("[^\n]*wibox.container.background[^\n]*attach PARENT[^\n]*") do
            floats[#floats + 1] = line
        end
        assert(#floats == 2, "the manual layout does not have two floats")
        check_box(floats[1], 10, 5, 30, 20)
        check_box(floats[2], 100, 10, 40, 20)
        ...
        io.stderr:write("[PASS] manual floats follow explicit and forced sizes\n")
        return true
    end,
    function(count)
        if count == 1 then
