-- Stock fair membership, orientation, whole-pixel allocation and input.
...
runner.run_async(function()
    ...
    local colors = {'ffcc0000', 'ff00cc00', 'ff0000cc', 'ffcccc00', 'ffcc00cc', 'ff00cccc', 'ff663300', 'ff336600', 'ff003366', 'ff660033'}
    s.selected_tag.layout = awful.layout.suit.tile
    for count = 0, 10 do
        if count > 0 then
            local report = os.tmpname()
            reports[count] = report
            awful.spawn {binary, 'FAIR_' .. count, report, '0', colors[count]}
            for _ = 1, 30 do
                clients[count] = utils.find_client_by_class('FAIR_' .. count)
    ...
    s.inspector = true
    async.sleep(0.15)
    example.save('fair-layout-inspector', awesome._clay_tree(s))
    s.inspector = false
    for _, c in ipairs(clients) do c:kill() end
    for _, report in ipairs(reports) do os.remove(report) end
    io.stderr:write('[PASS] both fair variants at 0..10 clients and two gaps; titlebars at 1/2/3 clients, original real input and solved configure equality\n')
    assert(#failures == 0, 'non-native fair declarations: ' .. table.concat(failures, ', '))
    runner.done()
end)
