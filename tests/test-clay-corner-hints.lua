-- Corner allocations contain surface minima; real input follows overlap paint.
...
runner.run_async(function()
    ...
    for count=1,3 do
        ...
        awful.spawn{binary,'CORNER_HINT_'..count,reports[count],count==1 and '900' or '0',colors[count]}
        for _=1,30 do
            clients[count]=utils.find_client_by_class('CORNER_HINT_'..count)
        ...
        if count==3 then
            ...
            for _,kind in ipairs{'nw','ne','sw','se'} do
                s.selected_tag.layout=awful.layout.suit.corner[kind]
                for _,gap in ipairs{0,8} do
                    ...
                    local x=kind:sub(2,2)=='w' and 0 or 640
                    local y=kind:sub(1,1)=='n' and 0 or 360
                    if g.x~=x+gap or g.y~=y+gap or g.width~=width-2-2*gap or g.height~=height-2-2*gap then
                        failures[#failures+1]=name..'-client-allocation'
                    end
                    local line=assert(dump:match('SURFACE CORNER_HINT_1 [^\n]+'))
