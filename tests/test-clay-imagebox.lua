...
local function imageboxes(list)
    ...
    for _, node in ipairs(list) do
        if node.image and node.line:find(" image 16x16 ", 1, true) then
            out[#out + 1] = {node=node, image=node}
        end
    end
    return out
end
...
local steps = {
    function(count)
        ...
        -- The fourth imagebox has no image, so it is not in the tree at all.
        assert(#ib == 3, "expected three imageboxes, got " .. #ib)
        local widgets = bar.widget:get_children()
        for _, widget in ipairs{widgets[1], widgets[2]:get_widget(), widgets[3]} do
            local occurrence = assert(bar._drawable._clay_wired[widget][1])
            assert(occurrence.element and occurrence.element.box,
                "original imagebox lost its allocated area")
        end
        assert(not bar._drawable._clay_wired[widgets[4]][1].element,
            "the empty imagebox retained an element")
