...
void bench_frame_stats_get(uint64_t *count, uint64_t *min_ns, uint64_t *max_ns,
    ...
    *count = bench_refresh_count;
    if (bench_frame_count == 0) { *min_ns = *max_ns = *avg_ns = *p99_ns = 0; return; }
    int n = bench_frame_count < BENCH_FRAME_HISTORY ? bench_frame_count : BENCH_FRAME_HISTORY;
    ...
    *min_ns = sorted[0]; *max_ns = sorted[n-1];
    uint64_t sum = 0; for (int i = 0; i < n; i++) sum += sorted[i];
    *avg_ns = sum / n;
    *p99_ns = sorted[(int)((n-1) * 0.99)];
}

void bench_frame_stats_reset(void) {
    bench_frame_index = 0; bench_frame_count = 0; bench_refresh_count = 0;
}
#endif
...
some_refresh(void)
{
	...
	if (bench_frame_count < BENCH_FRAME_HISTORY)
		bench_frame_count++;
#endif
