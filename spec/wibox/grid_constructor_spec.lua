...
local function host(g,width,height,fit)
    ...
    function self:frame()
        ...
        local boxes = solver.solve(tree,'helper-'..serial)
        clay._settle(tree,boxes)
        clay._publish(self,tree,boxes)
        return tree
    end
...
describe('grid helper lifecycle through production Clay',function()
    setup(solver.start)
    ...
    it('excludes overlays from sizing and reuses exact cells in production Clay',function()
        ...
        assert.is_true(on.float);assert.is_not_nil(on._attach._settle)
    ...
    it('builds shared real span anchors without changing tracks or border occupancy',function()
        ...
        assert.equals('grid.span-area',repeated[1].node._attach.name)
    ...
    it('anchors inside an existing span and excludes an overlap chain into holes',function()
        ...
        assert.equals('grid.span-area',anchor.name);assert.equals(83,anchor.w);assert.equals(40,anchor.h)
        assert.is_not_nil(solver.find(d._clay_tree,last)[1].node._attach._settle)
